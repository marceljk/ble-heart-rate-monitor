// node_modules/bluetooth-heart-rate/src/index.js
var intervalId = null;
var characteristicInstance = null;
function connect() {
...
function startHeartRateMeasurement(callback, intervalInSeconds = void 0) {
  if (!characteristicInstance) {
    console.log("Please connect to a device first.");
    return;
  }
  const throttledCallback = intervalInSeconds ? throttle(callback, intervalInSeconds * 1e3) : callback;
  characteristicInstance.startNotifications().then((_) => {
    characteristicInstance.addEventListener(
...
function parseHeartRate(value) {
  let format = (value.getUint8(0) & 1) === 0 ? "uint8" : "uint16";
  let heartRate = value.getUint8(1);
  if (format === "uint16") {
    heartRate = value.getUint16(
      1,
      /*littleEndian=*/
      true
    );
...
//# sourceMappingURL=bluetooth-heart-rate.js.map
